...
ssize_t BinaryDecoder::Decode(boost::asio::ip::tcp::socket &socket, RPC &msg) {
    try {
      ...
      if (!ReadExact(socket, &msgType, 1)) {
        std::cerr << "Failed to read message type byte" << std::endl;
        return -1;
      }

      // debug priunt statement
      std::cout << "Read message type byte: " << static_cast<int>(msgType)
                << std::endl;
      ...
      // Protect against malicious or corrupted messages by limiting payload
      // size
