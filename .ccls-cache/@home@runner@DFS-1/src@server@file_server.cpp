...
bool FileServer::Store(const std::string &key, std::istream &data) {
  ...
  *fileBuffer << data.rdbuf();
  ...
  for (auto peer : activePeers) {
    boost::asio::write(peer->socket(), encryptedBuffer.data());
  }
...
bool FileServer::handleStoreFile(const std::string &from,
  ...
  // Create socket stream for reading from peer
  network::SocketStream socketStream(peer->second->socket());
  ...
  auto bytesProcessed =
      crypto::copyDecrypt(opts_.encKey, socketStream, dataStream);
  if (bytesProcessed < 0) {
    return false;
  }

  // Write decrypted data to store
  auto result = store_->Write(id, key, dataStream);
  if (result < 0) {
    return false;
  }
...
bool FileServer::handleGetFile(const std::string &from,
  ...
  std::ostream socketStream(&buffer);

  // Encrypt and send the file
  auto bytesProcessed =
      crypto::copyEncrypt(opts_.encKey, *stream, socketStream);
  if (bytesProcessed < 0) {
    return false;
  }

  // Write the encrypted data to socket
  boost::asio::write(peer->second->socket(), buffer);
